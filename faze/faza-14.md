# Faza 14 — Podesavanja ekran

## Opis
Port podesavanja (tema i jezik) + prikaz auth statusa (profil info i odjava).
Web `PodesavanjaKlijent.tsx` ima samo temu i jezik. Mobile dodaje auth sekciju
(ime korisnika, email, odjava) jer nema zasebnog profil ekrana.

## Zavisnosti
Faza 3, Faza 4, Faza 5, Faza 6 (useAuth)

## Web referenca
```
src/app/[locale]/(main)/podesavanja/PodesavanjaKlijent.tsx
src/app/[locale]/(main)/podesavanja/page.tsx
...
## `app/(tabs)/podesavanja.tsx` — Podesavanja ekran

```tsx
// app/(tabs)/podesavanja.tsx
...
export default function PodesavanjaTab() {
  ...
  const { korisnik, ucitava, jeGost } = useAuth(); // jeGost je vec definisan u useAuth (Faza 6)

  const handleJezik = (noviJezik: Jezik) => {
    setJezik(noviJezik);
    // react-i18next menja jezik odmah — nema URL redirect kao na webu
  };
...
| Web | Mobile |
|-----|--------|
| `router.push('/podesavanja', { locale: noviJezik })` za promenu jezika | `i18n.changeLanguage(noviJezik)` — odmah menja bez navigacije |
| Nema auth sekciju na podesavanjima | Auth info + odjava u istom ekranu (nema profil stranice) |
| Nema odjavu u podesavanjima — ima zaseban profil link | `Alert.alert` za potvrdu odjave |
| `radio` input elementi | Pressable sa `accessibilityRole="radio"` |

## Sta izostaviti
- `router.push('/podesavanja', { locale })` — next-intl URL localization; mobile samo menja `i18n.changeLanguage`
- `generateMetadata` — ne postoji na mobilnom
- `sr-only` CSS klasa za radio inpute — koristiti direktno `accessibilityRole`
- Zasebni profil ekran — sve je na podesavanjima

## Commit
`feat: podesavanja — tema, jezik, auth status i odjava`

## Proveri pre commita
- [ ] Tema chip-ovi menjaju temu odmah (svetla/tamna/sistem)
- [ ] Jezik chip-ovi menjaju jezik odmah — UI se prevodi bez restarta
- [ ] Aktivan chip je zelen, ostali su beli/tamni
- [ ] Gost vidi "Nisi prijavljen" sa dugmadima Prijavi se / Registruj se
- [ ] Ulogovani vidi ime i email (iz `korisnik.user_metadata` / `korisnik.email`)
- [ ] Odjava prikazuje Alert za potvrdu
- [ ] Nakon odjave: `router.replace('/(tabs)')` (vraca na pocetnu bez auth ekrana u historiji)
- [ ] Dark mode izgleda ispravno na svim elementima
